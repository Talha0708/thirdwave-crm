                        ...
                        return (
                          <tr key={p._id} className="hover:bg-white/[0.02] transition-colors group">
                            ...
                            <td className="px-6 py-4 text-right flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                              ...
                              <button onClick={() => handleDelete(p._id)} disabled={deletingId === p._id} className="p-2 text-zinc-500 hover:text-red-400 hover:bg-red-950/30 rounded-lg transition-colors disabled:opacity-50">
        ...
        <Modal title={isEditing ? 'Mutate Data Model' : 'Define Data Model'} onClose={closeModal}>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <FormField label="Scale Configuration (Select Available Sizes)">
              ...
              <p className="text-[10px] text-zinc-500 mt-2 font-mono leading-relaxed">* Leave unselected for items without size parameters (e.g. Wallets).</p>
            </FormField>
            <div className="grid grid-cols-2 gap-4">
...
// ══════════════════════════════════════════════════════════════
//  Settings View
//  FIX: savePrompt now re-fetches from DB after saving so the
//       textarea always shows what is actually stored.
// ══════════════════════════════════════════════════════════════
const SettingsView = ({ showMessage }) => {
  ...
  // ✅ FIX: Extracted into a reusable function so we can call it after save too
  const fetchConfig = useCallback(async () => {
    try {
      const res = await api.get('/shop/config');
      if (res.data) setConfig(res.data);
    } catch {
      showMessage('Failed to fetch config.', 'error');
    } finally {
      setLoading(false);
    }
  }, [showMessage]);
  ...
  // ✅ FIX: After saving, re-fetch from DB so the textarea reflects
  //         exactly what was stored — no more stale-state on refresh.
  const savePrompt = async () => {
    setSaving(true);
    try {
      await api.put('/shop/config', { systemPrompt: config.systemPrompt });
      // Re-fetch so the displayed value is guaranteed to match the DB
      const res = await api.get('/shop/config');
      if (res.data) setConfig(res.data);
  ...
  return (
    <div className="space-y-6 max-w-5xl animate-in fade-in duration-500">
      ...
      <div className="bg-[#0A0A0A] rounded-2xl border border-white/10 p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl">
        ...
        <button
          onClick={toggleAI}
          className={`relative w-14 h-7 rounded-full transition-all duration-300 focus:outline-none ${config.isAIActive ? 'bg-white shadow-[0_0_15px_rgba(255,255,255,0.4)]' : 'bg-zinc-800 border border-white/10'}`}>
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-200 flex flex-col md:flex-row font-sans selection:bg-white/20">
      <style>{`
        ...
        .btn-primary { padding: 0.625rem 1.25rem; background-color: #ededed; color: #000; font-size: 0.875rem; font-weight: 600; border-radius: 0.5rem; display: flex; align-items: center; gap: 0.5rem; transition: all 0.2s; cursor: pointer; border: none; }
        .btn-primary:hover:not(:disabled) { background-color: #fff; box-shadow: 0 0 15px rgba(255,255,255,0.2); }
        .btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
        .btn-secondary { padding: 0.625rem 1.25rem; background-color: rgba(255,255,255,0.05); color: #e4e4e7; border: 1px solid rgba(255,255,255,0.1); font-size: 0.875rem; font-weight: 500; border-radius: 0.5rem; display: flex; align-items: center; gap: 0.5rem; transition: all 0.2s; cursor: pointer; }
