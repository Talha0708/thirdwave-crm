...
const ProductCatalog = () => {
  ...
  const [formData, setFormData] = useState({ 
      name: '', category: 'Shirt', price: '', stock: '', sizes: [], status: 'Active' 
  });
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      setFormData({ name: '', category: 'Shirt', price: '', stock: '', sizes: [], status: 'Active' });
            ...
            <div className="bg-[#0A0A0A] border border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl my-8">
                ...
                <form onSubmit={handleAddProduct} className="p-6 space-y-4">
                    <div>
                        ...
                        <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-[#111111] border border-zinc-800 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-indigo-500/50" placeholder="e.g. Aurelian Premium Oxford Shirt" />
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-sm text-zinc-400 block mb-1.5">Category</label>
                            <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full bg-[#111111] border border-zinc-800 rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:border-indigo-500/50">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-sm text-zinc-400 block mb-1.5">Initial Stock</label>
                    ...
                    <button type="submit" disabled={addLoading} className="w-full mt-6 flex items-center justify-center gap-2 py-3 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 transition-all active:scale-[0.98] disabled:opacity-70">
                        {addLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Save Product to Database'}
                    </button>
                </form>
            </div>
        </div>
