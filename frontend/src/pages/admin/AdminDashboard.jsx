...
const AdminDashboard = () => {
  ...
  // অ্যাডমিনের জন্য ডামি স্ট্যাটিস্টিকস
  const stats = [
    { label: 'Total MRR (Revenue)', value: '৳ 2,45,000', trend: '+15.2%', isPositive: true, icon: DollarSign, color: 'text-emerald-400', bg: 'bg-emerald-400/10', border: 'border-emerald-400/20' },
    { label: 'Active Clients', value: '24', trend: '+3', isPositive: true, icon: Users, color: 'text-blue-400', bg: 'bg-blue-400/10', border: 'border-blue-400/20' },
  ...
  // রিসেন্ট ক্লায়েন্ট লিস্ট
  const recentClients = [
    { id: 'CLI-092', name: 'Aurelian Menswear', plan: 'Enterprise', status: 'Active', mrr: '৳ 15,000' },
    { id: 'CLI-091', name: 'Urban Fit BD', plan: 'Pro', status: 'Active', mrr: '৳ 8,000' },
    { id: 'CLI-090', name: 'Style Echo', plan: 'Basic', status: 'Onboarding', mrr: '৳ 4,000' },
    { id: 'CLI-089', name: 'Luxe Attire', plan: 'Pro', status: 'Active', mrr: '৳ 8,000' },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto pb-10">
          ...
          <div key={index} className="bg-[#0A0A0A] border border-zinc-800 p-6 rounded-2xl relative overflow-hidden group hover:border-zinc-700 transition-colors">
            ...
            <div className="relative z-10 flex justify-between items-start mb-4">
              ...
              <span className={`flex items-center text-xs font-semibold px-2 py-1 rounded-full ${stat.isPositive ? 'text-emerald-400 bg-emerald-400/10' : 'text-red-400 bg-red-400/10'}`}>
                {stat.trend} {stat.trend !== 'Stable' && <ArrowUpRight className="w-3 h-3 ml-1" />}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* ─── Recent Clients Table (Left Side - 2 Cols) ─── */}
                  ...
                  <tr key={client.id} className="hover:bg-zinc-900/30 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-zinc-800 flex items-center justify-center font-bold text-white border border-zinc-700">
                    ...
                    <td className="px-6 py-4 font-medium text-zinc-300">{client.mrr}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider ${
                        client.status === 'Active' ? 'text-emerald-400 bg-emerald-400/10 border border-emerald-400/20' : 'text-amber-400 bg-amber-400/10 border border-amber-400/20'
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${client.status === 'Active' ? 'bg-emerald-400' : 'bg-amber-400'}`}></span>
                        {client.status}
                      </span>
                    </td>
        ...
        {/* ─── Server Infrastructure (Right Side - 1 Col) ─── */}
