...
 * Version: 2.1.0 — Enterprise Grade (Full Debug Logs)
 * ============================================================
 */
'use strict';
...
// ── Webhook stricter limiter (prevent FB ping storms) ──
...
                     || password === user.password; // legacy plain-text fallback
...
// ══════════════════════════════════════════════════════════════
//  9. AI Engine — Gemini 3.1 Flash-Lite (GA)
// ══════════════════════════════════════════════════════════════
const PLAN_LIMITS = {
...
// Greedy match — handles nested {} in address fields
...
    // ── Chat History: keep last 10 turns (20 messages) ──
...
            {
                messaging_type: 'RESPONSE',          // ✅ Required for v19.0+
...
// ══════════════════════════════════════════════════════════════
//  12. Queue Worker (5-second poll)
//  NOTE: On Vercel, this runs per-instance. For multi-instance
//        reliability, migrate queue to Redis / BullMQ.
// ══════════════════════════════════════════════════════════════
setInterval(async () => {
...
        if (!shop) {
            console.log(`❌ No shop found for Page ID: "${pageId}" — check metaPageId in DB.`);
            return;
        }
        if (!shop.isAIActive) {
            console.log(`⚠️ AI is OFF for shop: ${shop.shopName}`);
            return;
        }

        const messaging = body.entry?.[0]?.messaging?.[0];
        if (!messaging) {
            console.log('⚠️ No messaging object in payload — likely a delivery/read receipt.');
            return;
        }
...
        console.log(`💬 PSID: ${psid} | Text: "${text}" | HasImage: ${!!imgUrl}`);
...
        console.log(`🤖 AI Response (first 100 chars): "${aiResponse.slice(0, 100)}"`);

        if (aiResponse === 'QUEUED') {
            console.log('⏳ Rate limited — message queued.');
...
            console.log('⚠️ Not a WA business account event — ignoring.');
            return;
        }
...
            console.log('⚠️ No message in payload (likely a status update) — ignoring.');
            return;
        }
...
        console.log(`🔍 WA Phone Number ID: ${phoneNumberId} | From: ${fromNumber}`);

        const shop = await Shop.findOne({ whatsappPhoneNumberId: phoneNumberId }).lean();

        if (!shop) {
            console.log(`❌ No shop found for WA Phone Number ID: "${phoneNumberId}"`);
            return;
        }
        if (!shop.isAIActive) {
            console.log(`⚠️ AI is OFF for shop: ${shop.shopName}`);
...
        console.log(`💬 WA Text: "${text}" | HasImage: ${!!imgUrl}`);

        if (!text && !imgUrl) {
            console.log('⚠️ Unsupported WA message type — ignoring.');
            return;
        }
...
        if (aiResponse === 'QUEUED') {
            console.log('⏳ Rate limited — WA message queued.');
...
    console.log(`🚀 Thirdwave SaaS API v2.1.0 running on port ${PORT}`);
