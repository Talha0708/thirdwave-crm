...
async function connectDB() {
    ...
    dbPromise = mongoose.connect(process.env.MONGO_URI, {
        ...
        dbPromise = null; // reset so next request can retry
...
app.post('/api/products', authMiddleware, async (req, res) => {
    try {
        ...
        // Prevent duplicate product code within same shop
...
// ── RPM Tracker (in-memory; resets per serverless instance lifecycle) ──
...
// ── In-memory queue (Vercel serverless: each instance has its own queue) ──
const messageQueue = [];
...
async function getAurelianResponse(shop, psid, text, imgUrl = null, isFromQueue = false) {
    ...
    // ── Chat History: keep last 10 turns (20 messages), start from first user msg ──
    ...
    // Remove last message if it's from user (it's the current turn)
    ...
    try {
        ...
        // Persist chat history (keep last 20 messages = 10 turns)
        await Promise.all([
...
async function processOrderSync(shop, aiResponse) {
    ...
    if (!isValidPhone(phone) && phone !== 'Unknown') {
        console.warn(`⚠️ Order Sync: suspicious phone number "${phone}" — saving anyway`);
    }
...
async function sendFacebookMessage(shop, recipientId, text) {
    ...
    try {
        ...
        // Log full FB error response so we can debug 400s
...
app.post('/webhook/whatsapp', async (req, res) => {
    ...
    try {
        ...
        if (message.type === 'text') {
            ...
            // Download WA image via Graph API
            try {
...
app.listen(PORT, async () => {
    console.log(`🚀 Thirdwave SaaS API v2.0.0 running on port ${PORT}`);
